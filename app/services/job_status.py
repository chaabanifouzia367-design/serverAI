...
class JobStatusManager:
    ...
    @classmethod
    def get_all_jobs(cls, limit=50):
        """Get list of recent jobs with their status."""
        if redis_client is None:
            return []
        try:
            # Get job IDs from the sorted set (ordered by timestamp descending)
            job_ids = redis_client.zrevrange("job_timestamps", 0, limit - 1)
