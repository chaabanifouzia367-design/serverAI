...
def update_report_metadata(report_id, voxel_data=None, slice_count=None):
    update_fields = {}
    if voxel_data is not None:
        update_fields["voxel_data"] = voxel_data  # JSON column
    if slice_count is not None:
        update_fields["slice_count"] = slice_count  # JSON column

    if update_fields:
        get_supabase().table("report_ai").update({"metadata":update_fields}).eq("report_id", report_id).execute()
